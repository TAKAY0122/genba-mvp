...
/* ================= 表紙 ================= */
titleSlide("現場運営支援システム 操作説明書", "genba-mvp / Version 1.1 / 2026年8月(Googleログイン・2段階認証を追加)");

/* ================= 目次 ================= */
{
  const s = contentSlide("CONTENTS", "目次");
  const items = [
    "1. システム概要", "2. ログイン・チーム参加", "3. Googleログイン・2段階認証", "4. チーム作成(オーナー向け)",
    "5. Command Center(管理者向け)", "6. 勤務・休憩", "7. 配置管理(管理者向け)",
    "8. ポイント投票・バッジ", "9. AI提案", "10. チャット・通知",
    "11. マイページ", "12. プラン・課金", "13. 管理ページ(サイト管理者向け)", "14. よくある質問",
  ];
...
/* ================= 3. Googleログイン・2段階認証 ================= */
sectionSlide("03", "Googleログイン・2段階認証");
{
  const s = contentSlide("使い方", "Googleアカウントでログイン/2段階認証を設定する");
  bulletsBox(s, [
    "ログイン画面の「Googleでログイン」を押すと、Googleのログイン画面に移動して認証できます(パスワードの入力は不要)",
    "同じメールアドレスで既にアカウントをお持ちの場合は、自動的にそのアカウントと連携されます",
    "新規登録直後、2段階認証の設定案内が表示されます(その場で設定してもスキップして後で設定してもかまいません)",
    "設定する場合は、認証アプリ(Google Authenticator等)でQRコードを読み取り、表示された6桁コードを入力すると有効化されます",
    "有効化すると、次回以降のログイン時に認証アプリの6桁コードの入力が必要になります(なりすまし防止)",
    "認証アプリが手元にない場合に備えて、バックアップコード(8個・1回限り使用)が発行されます。必ず控えておいてください",
    "2段階認証はマイページの「セキュリティ設定」からいつでもON/OFFを切り替えられます",
  ], 0.7, 1.9, 11.9, 4.9, { fontSize: 15 });
}

/* ================= 4. チーム作成 ================= */
sectionSlide("04", "チーム作成(オーナー向け)");
{
...
/* ================= 5. Command Center ================= */
sectionSlide("05", "Command Center(管理者向け)");
{
...
/* ================= 13. 管理ページ ================= */
sectionSlide("13", "管理ページ(サイト管理者向け)");
{
