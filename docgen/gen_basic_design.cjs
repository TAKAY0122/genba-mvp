...
sections.push(...revisionHistory([
  ...
  ["v1.1", "2026年8月", "Googleアカウントログイン・2段階認証(TOTP)の追加(migration 0007)に伴い更新"],
]));
...
sections.push(makeTable(
  ...
  [
    ["認証・参加", "アカウント登録/ログイン、ゲスト参加(QR/URL/コード)、Googleアカウントログイン(OAuth 2.0)、2段階認証(TOTP)の設定・解除"],
...
sections.push(makeTable(
  ...
  [
    ["ログイン/新規登録", "認証・参加(パスワード/Googleログイン)", "全員"],
    ["2段階認証設定", "認証・参加(TOTP設定・バックアップコード発行、マイページから管理)", "アカウント保有者"],
...
sections.push(makeTable(
  ...
  [
    ...
    ["Google OAuth 2.0", "サーバー主導のAuthorization Codeフロー", "アカウント連携ログイン(トークン交換・ユーザー情報取得は全てサーバー側で実施)"],
...
sections.push(makeTable(
  ...
  [
    ...
    ["セキュリティ", "パスワードはPBKDF2ハッシュ化、Stripe Webhookは署名検証、監査ログは追記のみ。TOTPシークレットは非公開、バックアップコードはハッシュ化して保存。Google OAuthはCSRF対策のstateパラメータで検証"],
  ],
));
