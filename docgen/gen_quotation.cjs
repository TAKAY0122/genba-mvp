...
sections.push(...revisionHistory([
  ...
  ["v1.2", "2026年8月", "Googleアカウントログイン・2段階認証(TOTP)機能追加分の工数を計上。あわせて2026年時点の市場相場を再調査し、既存の単価水準(SE相当80万円/月・実装担当75万円/月)に変更が無いことを確認した"],
]));
...
sections.push(makeTable(
  ...
  [
    ...
    ["認証機能拡張", "Googleログイン(OAuth 2.0)連携、2段階認証(TOTP・バックアップコード)実装、関連画面追加、DBスキーマ拡張(3テーブル追加)", "0.65人月", "500,000円"],
    ["結合・受入テスト", "wrangler devによるE2E検証、不具合修正", "0.5人月", "380,000円"],
    ["リリース・ドキュメント整備", "本番デプロイ、開発ドキュメント一式作成", "0.4人月", "300,000円"],
    ["小計", "", "6.45人月", "4,920,000円"],
    ["消費税(10%)", "", "", "492,000円"],
    ["合計", "", "", "5,412,000円"],
  ],
));
...
sections.push(p("定額保守プランは、システム保守費用の市場相場である「初期開発費の年15〜20%」を基準に算定した(開発費4,920,000円 × 17.5% ÷ 12ヶ月)。都度対応の軽微修正は、実働時間ベースの下限額として別途設定する。"));
sections.push(makeTable(
  [{ text: "項目", width: 3200 }, { text: "金額(税別)", width: 2800 }, { text: "備考", width: 3800 }],
  [
    ["定額保守プラン(月額・任意)", "71,000円/月", "初期開発費の年15〜20%が市場相場。問い合わせ対応・軽微な不具合修正・監視を含む"],
...
sections.push(bullet("本見積は現行仕様(機能一覧書 v1.2、Googleログイン・2段階認証を含む)を対象範囲とする"));
